...
void ofApp::update() {
	for (auto& kv : deviceScopes) {
		...
		wifiHost.readData(devId, packets);

		map<pair<int, int>, vector<vector<float>>> scopeUpdates;

		for (auto& pkt : packets) {
			auto split = ofSplitString(pkt, ",");
			EmotiBitPacket::Header h;
			if (!EmotiBitPacket::getHeader(split, h)) continue;
			...
			for (int i = EmotiBitPacket::headerLength; i < split.size(); ++i) {
				dataVec.push_back(ofToFloat(split[i]));
			}
